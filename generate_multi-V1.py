...
def arg_parse():
    ...
    parser.add_argument("--i", dest="Image_Dir", default='./Test/Image/',help="Image directory for Datasets.")
    parser.add_argument("--m", dest="Mask_Dir", default='./Test/Mask/', help="Mask directory for Datasets.")
    parser.add_argument("--o", dest="Out_Dir", default='./Test/Output/',help="Output directory")
    
    parser.add_argument("--positive_brush", dest="positive_brush", default=[0,255,0],help="positive_brush")
    parser.add_argument("--negative_brush", dest="negative_brush", default=[0,0,255],help="negative_brush")
...
#     for f in images:
    ...
    rand_parms = zip(num_fg_stroke_pts,num_bg_stroke_pts,num_edge_seeds_fg_n_samples,num_edge_seeds_bg_n_samples)
    ...
    for iter,(num_fg_seeds,num_bg_seeds,num_edge_seeds_fg,num_edge_seeds_bg) in enumerate(rand_parms):
        ...
        fg_map_img = cv2.polylines(fg_map_img,[fg_stroke_pts],False,positive_brush,thickness=15)
        ...
        bg_map_img = cv2.polylines(bg_map_img,[bg_stroke_pts],False,negative_brush,thickness=15)
        #draw original image
...
#         fid.write("Checkpoint3:{}".format(cp3)+"\n")
        ...
        dismap_fg = cv2.distanceTransform(fg_mask, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)

        dismap_bg = cv2.distanceTransform(bg_mask, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
